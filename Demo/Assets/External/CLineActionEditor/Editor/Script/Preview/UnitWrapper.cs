...
namespace SuperCLine.ActionEngine
{
    ...
    public class UnitWrapper : Singleton<UnitWrapper>
    {
        ...
        public bool BuildUnit(IProperty property)
        {
            ...
            GameObject go = ResourceMgr.Instance.LoadObject<GameObject>(prefabname);
            if (go == null)
            {
            ...
            mAnim = mUnit.GetComponentInChildren<Animator>();
        ...
        public void BuildWeapon(WeaponProperty wp)
        {
            ...
            GameObject go = ResourceMgr.Instance.LoadObject<GameObject>(wp.Prefab);
            if (go == null) return;
        ...
        public void DoWeaponAttack(bool left)
        {
            ...
            else
            {
                ...
                if (mWeapon == null)
                {
                    Debug.LogError("--- ---  先绑定武器，  或者移除WeaponAttack Event");
                }
                Helper.AddChild(mWeapon, hand.gameObject);
            }
        }
