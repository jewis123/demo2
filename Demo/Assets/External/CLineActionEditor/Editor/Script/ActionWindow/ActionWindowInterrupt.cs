...
namespace SuperCLine.ActionEngine.Editor
{
    ...
    internal sealed partial class ActionWindow
    {
        ...
        private void SaveActionInterrupt()
        {
            ...
            string path = string.Format("{0}ActionInterrupt/{1}.json", ResConfig.RootDataPath, UnitWrapper.Instance.ActionGroupName);
        ...
        private void DeserializeInterrupt()
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("{0}ActionInterrupt/{1}.json", ResConfig.RootDataPath, groupName);

            if (File.Exists(sb.ToString()))
            {
                TextAsset t = ResourceMgr.Instance.LoadObject<TextAsset>(ResConfig.RootDataResPath + ResConfig.ActionInterruptDataPath + groupName);
