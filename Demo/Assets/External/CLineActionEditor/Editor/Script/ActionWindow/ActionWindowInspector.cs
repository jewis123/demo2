    ...
    internal sealed partial class ActionWindow
    {
        ...
        private void BackupsResource(string path)
        {
            if (File.Exists(path))
            {
                ...
                string filePath = path.Replace(ResConfig.RootDataPath, "");
        ...
        {
            var path = ResConfig.PathPrefix+filename;
            if (File.Exists(path))
            {
                TextAsset text = ResourceMgr.Instance.LoadObject<TextAsset>(path);
                if (text == null)
                {
        ...
        private void Deserialize()
        {
            ...
            DeserializeProperty<PlayerProperty>(ResConfig.PlayerPropertyFile, actorPlayerTree);
            DeserializeProperty<MonsterProperty>(ResConfig.MonsterPropertyFile, actorMonsterTree);
            DeserializeProperty<WeaponProperty>(ResConfig.WeaponPropertyFile, actorWeaponTree);
            DeserializeProperty<AISwitch>(ResConfig.AISwitchFile, actorAITree);
            DeserializeProperty<BuffFactoryProperty>(ResConfig.BuffFactoryPropertyFile, actorBuffTree);
        }
