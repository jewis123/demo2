...
namespace SuperCLine.ActionEngine
{
    ...
    public class NormalEmitComponent : EmitComponent
    {
        ...
        protected virtual Vector3 CalcStartPosition(bool random = false, bool randomonnavmesh = false)
        {
            ...
            {
                dummy = AH.Owner.UObject.Find(mProperty.Dummy);
            }
            ...
            switch (mProperty.PosType)
            {
                case EEmitterPosType.EEPT_AttackerCurrentPosAndDir:
                    {
                        pos = dummy != null ? dummy.position:AH.Owner.Position;
                        fward = dummy != null ? dummy.forward : AH.Owner.UObject.forward;
                    }
                    break;
                case EEmitterPosType.EEPT_AttackerCurrentPosAndTargetDir:
                    {
                        pos = dummy != null ? dummy.position:AH.Owner.Position;
                        fward = AH.Owner.Target != null
                            ? new Vector3(AH.Owner.Target.Position.x, 0f, AH.Owner.Target.Position.z) - new Vector3(AH.Owner.Position.x, 0f, AH.Owner.Position.z)
                            : AH.Owner.UObject.forward;
                    }
                    break;
            }
