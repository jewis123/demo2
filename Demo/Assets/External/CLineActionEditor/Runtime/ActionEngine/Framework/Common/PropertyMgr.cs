...
namespace SuperCLine.ActionEngine
{
    ...
    public sealed class PropertyMgr : Singleton<PropertyMgr>
    {
        ...
        {
            Load(ResConfig.PlayerPropertyFile, (PlayerProperty p) => { mPlayerPropertyHash.Add(p.ID, p); });
            Load(ResConfig.MonsterPropertyFile, (MonsterProperty m) => { mMonsterPropertyHash.Add(m.ID, m); });
            Load(ResConfig.WeaponPropertyFile, (WeaponProperty w) => { mWeaponPropertyHash.Add(w.ID, w); });
            Load(ResConfig.BuffFactoryPropertyFile, (BuffFactoryProperty b) => { mBuffPropertyHash.Add(b.ID, b); });

            string[] fileList = ResourceMgr.Instance.GetFilePaths(ResConfig.RootDataPath + ResConfig.ActionDataPath);
        ...
        {
            TextAsset text = ResourceMgr.Instance.LoadObject<TextAsset>(name);
            if (text == null)
            {
