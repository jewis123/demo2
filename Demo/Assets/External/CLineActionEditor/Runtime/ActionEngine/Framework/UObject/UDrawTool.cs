...
namespace SuperCLine.ActionEngine
{
    ...
    public class UDrawTool : MonoBehaviour
    {
        ...
        public void DrawCircle(Transform t, Vector3 center, float radius, Color color)
        {
            LineRenderer lr = GetLineRenderer(t);
            lr.startColor = color;
            lr.endColor = color;
        ...
        private GameObject CreateMesh(List<Vector3> vertices, Color color, ref Mesh mesh)
        {
            int[] triangles;
            if (mesh == null)
            {
                mesh = new Mesh();
                mesh.name = "UDrawTools Mesh";
            }
        ...
        public void DrawSectorSolid(Transform t, Vector3 center, float angle, float radius, Color color, ref Mesh mesh)
        {
            int pointAmount = 100;  
            float eachAngle = angle / pointAmount;
            Vector3 forward = t.forward;
            ...
            CreateMesh(vertices, color,ref mesh);
        }

        public GameObject DrawCircleSolid(Transform t, Vector3 center, float radius, Color color, ref Mesh mesh)
        {
            int pointAmount = 100; 
            float eachAngle = 360f / pointAmount;
            Vector3 forward = t.forward;
            ...
            return CreateMesh(vertices, color, ref mesh);
        }

        public void DrawRectangleSolid(Transform t, Vector3 bottomMiddle, float length, float width, Color color, ref Mesh mesh)
        {
            List<Vector3> vertices = new List<Vector3>();
        ...
        public void DrawRectangleSolid2D(Transform t, float distance, float length, float width, Color color, ref Mesh mesh)
        {
            List<Vector3> vertices = new List<Vector3>();
