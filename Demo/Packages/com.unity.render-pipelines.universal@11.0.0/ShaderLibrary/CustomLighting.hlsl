...
//temp include shdaerCode    TODO: Code refactoring
...
float D_GGX_TR(half3 N, half3 H, float a)
{
    float a2     = a*a;
...
{
    //TODO 根据项目定制化magicNumber 和 Remap
...
half DirectBRDFSpecular(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS)
{
    ...
    //风格化高光效果尝试
    // #ifndef _USE_RAMPTEXTURE
    // specularTerm = smoothstep(_RampThreshold - _RampSmoothing, _RampThreshold + _RampSmoothing, specularTerm);
    // #endif
    #ifdef _USE_STYLIZED_SPECULAR
    half r = sqrt(brdfData.roughness) * 0.85 + 1e-4h;
    //根据粗糙度 构造风格化高光  后期根据需求修改  风格化高光会抹除材质属性
...
half3 DecodeHDREnvironmentModified(half4 encodedIrradiance)
{
...
#else
    half3 irradiance = DecodeHDREnvironmentModified(encodedIrradiance);
#endif
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData)
{
    ...
    //D项
    // return D_GGX_TR(inputData.normalWS, halfDir , 1- surfaceData.smoothness);
    ...
    half3 physicalCol = LightingPhysicallyBased(brdfData, brdfDataClearCoat,
                                     mainLight,
                                     inputData.normalWS, inputData.viewDirectionWS,
                                     surfaceData.clearCoatMask, specularHighlightsOff);
    // return half4(physicalCol,1);
...
half3 StylizedFresnel(half nv, half ndl, half rimMin, half rimMax, half rimStrength, half3 rimColor, half roughness)
{
    half rim = 1 - nv;
    rim = smoothstep(rimMin, rimMax, rim) * rimStrength * saturate(1.33 - roughness);
    return rim * ndl * rimColor;
}

half3 StyilzedLightingPhysicallyBased(BRDFData brdfData, BRDFData brdfDataClearCoat,
    half3 lightColor, half3 lightDirectionWS, half lightAttenuation,
    half3 normalWS, half3 viewDirectionWS,
    half clearCoatMask, bool specularHighlightsOff, half nl)
{
    half NdotL = nl;
    half3 radiance = lightColor * (lightAttenuation * NdotL);
...
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, float clearCoatMask,
                         half3 bakedGI, half occlusion,
                         half3 normalWS, half3 viewDirectionWS, half ndv)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = ndv;
    half fresnelTerm = Pow4(1.0 - NoV);
...
half3 StyilzedGlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, float clearCoatMask,
                         half3 bakedGI, half occlusion,
                         half3 normalWS, half3 viewDirectionWS, half ndv, half warpedAtten, out half fresnelTerm)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = ndv;
    ...
    // return brdfData.diffuse;
    // giDiffuse = lerp(giDiffuse, giDiffuse * warpedAtten + (1 - warpedAtten) * OverlayBlendMode(giDiffuse, _CustomShadowColor), _CustomShadowLerpFactor);
    // return (1 - warpedAtten) * OverlayBlendMode(giDiffuse, _CustomShadowColor);
    half3 giSpecular= indirectSpecular * EnvironmentBRDFSpecular(brdfData, fresnelTerm);

    half3 color = giDiffuse + giSpecular;
...
half3 OverlayBlend(half3 color, half3 blendCol)
{
    return lerp(1 - 2 * (1 - color.rgb) * (1 - blendCol.rgb), 2 * color.rgb * blendCol.rgb, step(color.rgb, 0.5));
}
//↑↑↑↑↑↑↑↑↑↑待整理区域↑↑↑↑↑↑↑↑↑↑

half4 FragmentStyilzedPBR(InputDataNew inputData, SurfaceData surfaceData)
{
#ifdef _SPECULARHIGHLIGHTS_OFF
    bool specularHighlightsOff = true;
#else
    ...
    mainLight.shadowAttenuation = min(mainLight.shadowAttenuation, surfaceData.occlusion);
    // #if defined(_SCREEN_SPACE_OCCLUSION)
    //     AmbientOcclusionFactor aoFactor = GetScreenSpaceAmbientOcclusion(inputData.normalizedScreenSpaceUV);
    //     mainLight.color *= aoFactor.directAmbientOcclusion;
    //     surfaceData.occlusion = min(surfaceData.occlusion, aoFactor.indirectAmbientOcclusion);
...
#if defined(_USE_TOONRAMP)
    NoLWarped = WrapRampNL(NoL, _RampThreshold, _RampSmoothing);
#endif

    half NoV = saturate(dot(inputData.normalWS, inputData.viewDirectionWS));
    
    half lightAtten = /*ramp.r */ mainLight.distanceAttenuation * mainLight.shadowAttenuation;

    half warpedAtten = NoLWarped * lightAtten;
    half fresnelTerm = 0;
    
    MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI);
    
    half3 gi = StyilzedGlobalIllumination(brdfData, brdfDataClearCoat, surfaceData.clearCoatMask,
                                     inputData.bakedGI, /*surfaceData.occlusion*/ 1,
                                     inputData.normalWS, inputData.viewDirectionWS, NoV,warpedAtten, fresnelTerm);

    half3 physicalCol = StyilzedLightingPhysicallyBased(brdfData, brdfDataClearCoat,
                                     mainLight.color,mainLight.direction,lightAtten,
                                     inputData.normalWS, inputData.viewDirectionWS,
                                     surfaceData.clearCoatMask, specularHighlightsOff, NoLWarped);
    half3 color = gi + physicalCol;
    // return half4(physicalCol,1);
    
    /*=====================Custom Shadow========================*/
    // half shadowAtten = max(fresnelTerm, lightAtten* NoLWarped);

    color = lerp(color, color * NoLWarped + (1 - NoLWarped) * _BaseShadowColor, _BaseShadowColor.a);
    
    half3 overlayColor = OverlayBlend(_CustomShadowColor.rgb, color);
    color = lerp(color, color * lightAtten + (1 - lightAtten) * overlayColor, NoL * _CustomShadowColor.a);
    
    half3 final_rimColor = 0;
    /*=====================Stylized Fresnel========================*/
#ifdef _USE_STYLIZED_FRESNEL
    //TODO  开启风格化菲涅尔之后  试试能否省去或者替代传统Fresnel的计算
    final_rimColor += StylizedFresnel(NoV,NoL,_RimMin,_RimMax,_RimStrength,_RimColor, brdfData.roughness);
#endif

    //先干掉这个功能了  需要Pre-Depth
#ifdef _USE_SCREENSPACE_RIM
    half3 positionVS = inputData.positionVS;
    half3 samplePositionVS = half3(positionVS.xy + _RimOffset.xy * _RimWidth, positionVS.z);
    ...
    float depth = inputData.positionNDC.z / inputData.positionNDC.w;
    float linearEyeDepth = LinearEyeDepth(depth, _ZBufferParams);
    // float offsetDepth = SAMPLE_TEXTURE2D_X(_CameraDepthTexture, sampler_CameraDepthTexture, samplePositionVP).r; 
    float offsetDepth = SAMPLE_TEXTURE2D_X(_CharacterDepthTexture, sampler_CharacterDepthTexture, samplePositionVP).r; 
    float linearEyeOffsetDepth = LinearEyeDepth(offsetDepth, _ZBufferParams);
    float depthDiff = (linearEyeOffsetDepth - linearEyeDepth);
    depthDiff = step(_RimThresh, depthDiff);
    // return depthDiff;
    final_rimColor += depthDiff * _RimColor * _RimIntensity;
    
#endif
    
    color += final_rimColor + surfaceData.emission;
