#ifndef UNIVERSAL_STANDARD_INPUT_INCLUDED
#define UNIVERSAL_STANDARD_INPUT_INCLUDED
...
TEXTURE2D(_CharacterDepthTexture); SAMPLER(sampler_CharacterDepthTexture);
...
half WrapRampNL(half nl, half threshold, half smoothness)
{
    ...
    // #endif
    #if _USE_RAMPTEXTURE
    nl = SAMPLE_TEXTURE2D(_Ramp, sampler_Ramp, half2(nl, nl)).r;
    #else
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    half4 channelMap = SAMPLE_TEXTURE2D(_MetallicGlossMap, sampler_MetallicGlossMap, uv);
    channelMap.a *= _Smoothness;
    
    outSurfaceData.albedo = albedoAlpha.rgb * _BaseColor.rgb;
    
    outSurfaceData.metallic = channelMap.r;
    outSurfaceData.specular = half3(0.0h, 0.0h, 0.0h);

    outSurfaceData.smoothness = channelMap.a;
    outSurfaceData.normalTS = SampleNormal(uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), _BumpScale);
    outSurfaceData.occlusion = lerp(1, channelMap.b, _OcclusionStrength);

    //TODO: 添加_Emission宏 如果复杂度允许
    outSurfaceData.emission = _EmissionScale * channelMap.g * _EmissionColor.rgb;
...
half3 OverlayBlendMode(half3 baseColor, half3 blendColor) {
    half3 result;
    result.r = OverlayBlendMode(baseColor.r,blendColor.r);
    result.g = OverlayBlendMode(baseColor.g,blendColor.g);
    result.b = OverlayBlendMode(baseColor.b,blendColor.b);
    return result;
}
