...
namespace UnityEditor.Timeline
{
    class TimelineTrackGUI : TimelineGroupGUI, IClipCurveEditorOwner, IRowGUI
    {
        ...
        static class Styles
        {
            ...
            public static readonly GUIContent trackNameBtnOnTooltip = DirectorStyles.TrTextContent(string.Empty, "Collapse Track Name");
            public static readonly GUIContent trackNameBtnOffTooltip = DirectorStyles.TrTextContent(string.Empty, "Expand Track Name");
        ...
        {
            get { return track.GetShowTrackName(); }
        }
        ...
        Rect DrawGeneralSuite(WindowState state, Rect rect)
        {
            ...
            var padding = DrawButtonSuite(4, ref rect);
            ...
            rect.x -= buttonWidth;
            DrawShowTrackNameButton(rect, state);
            rect.x -= buttonWidth;
        ...
        void DrawTrackBinding(Rect rect, Rect headerRect)
        {
            ...
            //     rect = new Rect(originalRect.x, originalRect.y, originalRect.width, originalRect.height * 0.5f);
            // else
            //     rect = originalRect;
            //
            ...
            // }
            //
            // rect = new Rect(originalRect.x,originalRect.y+originalRect.height*0.5f,originalRect.width,originalRect.height*0.5f);
            if (rect.height > 50)
            {
                var originalRect = rect;
                if (m_TrackDrawData.m_ShowTrackBindings)
...
// if we are editing, supply the entire width of the header
                ...
                rect = new Rect(originalRect.x,originalRect.y+originalRect.height*0.5f,originalRect.width,originalRect.height*0.3f);
                
                if (m_TrackDrawData.m_ShowTrackBindings)
                {
                    DoTrackBindingGUI(rect,true);
                }
            }
            else
            {

                if (m_TrackDrawData.m_ShowTrackBindings && (!showTrackName && !EditMode.showAllTrackName))
                {
                ...
                textStyle.normal.textColor =
                    SelectionManager.Contains(track) ? Color.white : m_Styles.customSkin.colorTrackFont;
                ...
                trackName = EditorGUI.DelayedTextField(rect, GUIContent.none, track.GetInstanceID(), track.name,
                    textStyle);
        ...
        void DrawShowTrackNameButton(Rect rect, WindowState state)
        {
            var markersShown = showTrackName;
            
            
            var style = TimelineWindow.styles.trackNameButton;

            EditorGUI.BeginChangeCheck();
            var tooltip = markersShown ? Styles.trackNameBtnOnTooltip : Styles.trackNameBtnOffTooltip;
            var toggleMarkers = GUI.Toggle(rect, markersShown, tooltip, style);
            
            if (EditorGUI.EndChangeCheck())
                track.SetShowTrackName(toggleMarkers);
        }
        ...
        void DoTrackBindingGUI(Rect rect,bool flowTop = false)
        {
            var bindingRect = 
                !flowTop?
